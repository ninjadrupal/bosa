...
shared_examples "comments" do
  ...
  it "allows user to sort the comments", :slow do
    ...
    # --- start of bosa patch -----------------------------------------------------------------------------------------
    # Accept all cookies to hide the popup (and it reloads the page after that)
    # A popup is blocking the `click_link "Best rated"` command from finding the link (it overlaps the link)
    ...
    within ".comments" do
      within ".order-by__dropdown" do
        # --- start of bosa patch -------------------------------------------------------------------------------------
        # Skip this as we have a change to open the dropdown on hover, so it is already opened by the command above
        #
