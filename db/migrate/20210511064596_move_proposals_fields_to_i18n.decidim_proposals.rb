...
class MoveProposalsFieldsToI18n < ActiveRecord::Migration[5.2]
  def up
    delete_searchable_resources_with_unused_locales
  ...
  def delete_searchable_resources_with_unused_locales
    Decidim::Organization.all.each do |o|
      Decidim::SearchableResource.where(organization: o).where.not(locale: o.available_locales).delete_all
    end
  end
end
